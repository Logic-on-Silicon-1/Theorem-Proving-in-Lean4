...
표현식 `e`의 타입 `T`를 항상 `(e : T)`로 쓰여 지정할 수 있습니다. 이것은 Lean의 엘래버레이터(elaborator)에게 암시적 인수를 해결하려고 할 때 `e`의 타입으로 값 `T`를 사용하도록 지시합니다. 아래의 두 번째 예제 쌍에서, 이 메커니즘은 표현식 `id`와 `List.nil`의 원하는 타입을 지정하는 데 사용됩니다:
