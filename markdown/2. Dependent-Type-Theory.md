...
다음은 Lean에서 객체를 선언하고 타입을 확인하는 방법의 예제입니다.

`/-`와 `-/` 사이의 모든 텍스트는 Lean에서 무시되는 주석 블록을 구성합니다. 마찬가지로, 두 개의 대시 `--`는 해당 줄의 나머지 부분이 무시되는 주석을 나타냅니다. 주석 블록은 중첩될 수 있으므로, 많은 프로그래밍 언어처럼 코드의 일부를 “주석 처리”할 수 있습니다.
...
위의 표현식들 각각이 `Type` 타입의 객체임을 볼 수 있습니다. 타입에 대한 새로운 상수를 선언할 수도 있습니다:

위의 예제에서 알 수 있듯이, `Type → Type → Type` 타입의 함수, 즉 데카르트 곱 `Prod`의 예제를 이미 보았습니다:
...
| 부분(sort) | `Prop` (`Sort 0`) | `Type` (`Sort 1`) | `Type 1` (`Sort 2`) | `Type 2` (`Sort 3`) | ... |
| --- | --- | --- | --- | --- | --- |
| 타입(type) | `True` | `Bool` | `Nat -> Type` | `Type -> Type 1` | ... |
| 항(term) | `True.intro` | `true` | `fun n => Fin n` | `fun (_ : Type) => Type` | ... |
...
그러나 일부 작업은 타입 우주에 대해 *다형적(polymorphic)*이어야 합니다. 예를 들어, `List α`는 `α`가 어느 타입 우주에 있든 상관없이 모든 타입 `α`에 대해 의미가 있어야 합니다. 이것이 함수 `List`의 타입 시그니처를 설명합니다:
...
Lean은 다음과 같이 표현식에서 함수를 만들기 위해 `fun` (또는 `λ`) 키워드를 제공합니다:

이 예제에서 `Nat` 타입을 추론할 수 있습니다:
...
수학적으로 일반적인 함수 연산의 몇 가지 예제는 람다 추상화 측면에서 설명할 수 있습니다:
...
함수를 매개변수로 전달할 수 있고, `f`와 `g` 이름을 지정하여 구현에서 이러한 함수를 사용할 수 있습니다:
...
람다 표현식의 일반적인 형태는 `fun (x : α) => t`이며, 여기서 변수 `x`는 “바운드 변수(bound variable)”입니다: 이는 진정한 자리 표시자이며, 그 “범위(scope)”는 표현식 `t`를 넘어 확장되지 않습니다. 예를 들어, 표현식 `fun (b : β) (x : α) => b`에서 변수 `b`는 이전에 선언된 상수 `b`와 아무 관계가 없습니다. 사실, 표현식은 `fun (u : β) (z : α) => u`와 같은 함수를 나타냅니다.
...
Note: This linter can be disabled with `set_option linter.unusedVariables false`x : Nat => true) 1 ``
...
나중에 이러한 항들이 어떻게 평가되는지 볼 것입니다. 지금은 이것이 종속 타입 이론의 중요한 특징이라는 점을 알아두세요: 모든 항은 계산적인 동작을 가지며, *정규화(normalization)*의 개념을 지원합니다. 원칙적으로 동일한 값으로 환원되는 두 항을 *정의적 동치(definitionally equal)*라고 합니다. Lean의 타입 검사기는 이들을 "동일한" 것으로 간주하며, Lean은 이러한 동일시를 인식하고 지원하기 위해 최선을 다합니다.
...
`def` 키워드가 새로운 명명된 객체를 선언하는 한 가지 중요한 방법을 제공한다는 것을 기억하세요.

`def double (x : Nat) : Nat :=
x + x`

다른 프로그래밍 언어에서 함수가 어떻게 작동하는지 알고 있다면, 이것은 더 친숙해 보일 수 있습니다. `double`이라는 이름은 `Nat` 타입의 입력 매개변수 `x`를 받는 함수로 정의되며, 호출의 결과는 `x + x`이므로 `Nat` 타입을 반환합니다. 그러면 이 함수를 사용하여 호출할 수 있습니다:

이 경우, `def`를 일종의 명명된 `fun`으로 생각할 수 있습니다. 다음은 같은 결과를 줍니다:

`def double : Nat → Nat :=
fun x => x + x``6#eval double 3`
...
Lean에 충분한 정보가 있어 추론할 수 있을 때 타입 선언을 생략할 수 있습니다. 타입 추론은 Lean의 중요한 부분입니다:
...
오른쪽 `bar`는 람다뿐만 아니라 모든 표현식이 될 수 있습니다. 따라서 `def`는 다음과 같이 단순히 값의 이름을 지정하는 데에도 사용될 수 있습니다:
...
`def` 내에 다른 더 흥미로운 표현식을 사용할 수 있습니다:

아마도 이것이 무엇을 할 것인지 추측할 수 있을 것입니다.
...
이제 좀 더 추상적으로 하기 위해, 타입 매개변수와 같은 인수를 지정할 수도 있습니다:
...
x * x``18#eval compose Nat Nat Nat double square 3`
...
Lean은 또한 `let` 키워드를 사용하여 “로컬” 정의를 도입할 수 있게 해줍니다. 표현식 `let a := t1; t2`는 `t2`에서 `a`의 모든 발생을 `t1`로 바꾸는 것의 결과와 정의적으로 동등합니다.
...
`let y := 2 + 2;
let z := y + y;
z * z : Nat#check let y := 2 + 2; let z := y + y; z * z`
...
`def compose (α β γ : Type) (g : β → γ) (f : α → β) (x : α) : γ :=
g (f x)``def doTwice (α : Type) (h : α → α) (x : α) : α :=
h (h x)``def doThrice (α : Type) (h : α → α) (x : α) : α :=
h (h (h x))`
...
`variable (α β γ : Type)``def compose (g : β → γ) (f : α → β) (x : α) : γ :=
g (f x)``def doTwice (h : α → α) (x : α) : α :=
h (h x)``def doThrice (h : α → α) (x : α) : α :=
h (h (h x))`
...
`variable (α β γ : Type)``variable (g : β → γ) (f : α → β) (h : α → α)``variable (x : α)``def compose := g (f x)``def doTwice := h (h x)``def doThrice := h (h (h x))``def compose : (α β γ : Type) → (β → γ) → (α → β) → α → γ :=
fun α β γ g f x => g (f x)#print compose`
...
fun α h x => h (h (h x))#print doThrice`
...
이들을 인쇄하면 세 가지 정의 그룹 모두 정확히 같은 효과를 가지고 있음을 보여줍니다.
...
섹션이 닫히면 변수는 범위를 벗어나고 더 이상 참조할 수 없습니다.
...
Lean은 정의들을 중첩된, 계층적 *네임스페이스(namespaces)*로 그룹화하는 능력을 제공합니다:

네임스페이스 `Foo`에서 작업하고 있다고 선언하면, 선언하는 모든 식별자는 “`Foo.`” 접두사가 있는 정규화된 이름을 가집니다. 네임스페이스 내에서 짧은 이름으로 식별자를 참조할 수 있지만, 네임스페이스를 끝내면 더 긴 이름을 사용해야 합니다. `section`과 달리, 네임스페이스는 이름이 필요합니다. 루트 레벨에는 하나의 익명 네임스페이스만 있습니다.
...
예를 들어, Lean은 목록과 관련된 정의와 정리를 `List` 네임스페이스로 그룹화합니다.

`open List` 명령을 사용하면 짧은 이름을 사용할 수 있습니다:

섹션과 같이, 네임스페이스는 중첩될 수 있습니다:

닫힌 네임스페이스는 나중에 다른 파일에서도 다시 열 수 있습니다:

섹션과 같이, 중첩된 네임스페이스는 열린 순서대로 닫아야 합니다. 네임스페이스와 섹션은 다른 목적을 제공합니다: 네임스페이스는 데이터를 구성하고 섹션은 정의에 삽입할 변수를 선언합니다. 섹션은 또한 `set_option`과 `open`과 같은 명령의 범위를 구분하는 데 유용합니다.
...
리스트의 맨 앞에 새로운 원소를 삽입하는 함수 `cons`를 작성하고 싶다고 가정하세요. `cons`의 타입은 무엇이어야 할까요? 이러한 함수는 *다형적(polymorphic)*입니다: `Nat`, `Bool`, 또는 임의의 타입 `α`에 대한 `cons` 함수가 같은 방식으로 작동하기를 기대합니다. 따라서 타입을 `cons`의 첫 번째 인수로 하는 것이 의미가 있으므로, 임의의 타입 `α`에 대해 `cons α`는 `α` 타입의 목록에 대한 삽입 함수입니다. 다시 말해, 모든 `α`에 대해, `cons α`는 원소 `a : α`와 리스트 `as : List α`를 받아서 새로운 리스트를 반환하는 함수이므로, `cons α a as : List α`를 가집니다.
...
이는 *종속 함수 타입(dependent function type)* 또는 **종속 화살표 타입(dependent arrow type)**의 예입니다. `α : Type`과 `β : α → Type`이 주어지면, `β`를 `α`에 대한 타입들의 족(family)으로 생각하세요. 즉, 각 `a : α`에 대해 타입 `β a`입니다. 이 경우, 타입 `(a : α) → β a`는 함수 `f`의 타입을 나타내며, 각 `a : α`에 대해 `f a`가 `β a`의 원소입니다. 다시 말해, `f`가 반환하는 값의 타입은 입력에 따라 달라집니다.
...
`universe u v``def f (α : Type u) (β : α → Type v) (a : α) (b : β a) : (a : α) × β a :=
⟨a, b⟩``def g (α : Type u) (β : α → Type v) (a : α) (b : β a) : Σ a : α, β a :=
Sigma.mk a b``def h1 (x : Nat) : Nat :=
(f Type (fun α => α) Nat x).2``5#eval h1 5`
...
`def h2 (x : Nat) : Nat :=
(g Type (fun α => α) Nat x).2``5#eval h2 5`
...
목록의 구현이 다음과 같다고 가정하세요:

그러면, 다음과 같이 `Nat`의 목록을 구성할 수 있습니다:
...
이것은 종속 타입 이론의 중심 특징입니다: 항들은 많은 정보를 가지고 있으며, 종종 그 정보 중 일부는 문맥에서 추론될 수 있습니다. Lean에서는 밑줄 `_`을 사용하여 시스템이 정보를 자동으로 채워야 함을 지정합니다. 이를 “암시적 인수(implicit argument)”라고 합니다.
...
이것은 `ident`의 첫 번째 인수를 암시적으로 만듭니다. 표기법적으로, 이것은 타입의 명세를 숨기므로, `ident`가 단순히 모든 타입의 인수를 받는 것처럼 보입니다. 사실, 함수 `id`는 표준 라이브러리에서 정확히 이런 방식으로 정의됩니다. 우리는 이름의 충돌을 피하기 위해 여기서만 비전통적인 이름을 선택했습니다.

변수는 또한 `variable` 명령으로 선언될 때 암시적으로 지정될 수 있습니다:
...
`e`라는 표현식의 타입 `T`는 항상 `(e : T)`라고 쓰면 명시할 수 있습니다. 이것은 암묵적인 인자 해석을 시도할 때 식 `e`의 타입으로 `T`라는 값을 사용하도록 Lean의 결합기(elaborator)에 지시합니다. 다음 두 번째 예제 세트에서 이 메커니즘을 적용하여 식 `id`와 `List.nil`의 원하는 타입을 지정하는 데 사용되었습니다:
...
표현식 `e`의 타입 `T`를 항상 `(e : T)`로 쓰여 지정할 수 있습니다. 이것은 Lean의 정교화기(elaborator)에게 암시적 인수를 해결하려고 할 때 `e`의 타입으로 값 `T`를 사용하도록 지시합니다. 아래의 두 번째 예제 쌍에서, 이 메커니즘은 표현식 `id`와 `List.nil`의 원하는 타입을 지정하는 데 사용됩니다:
...
그러나 때때로 우리는 함수에 대한 인수를 암시적으로 선언했지만 이제 인수를 명시적으로 제공하고 싶은 상황에 처할 수 있습니다. `foo`가 그러한 함수라면, 표기법 `@foo`는 모든 인수가 명시적으로 만들어진 같은 함수를 나타냅니다.
