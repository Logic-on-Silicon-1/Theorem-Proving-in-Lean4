...
```
#check let y := 2 + 2; y * y#eval  let y := 2 + 2; y * ydef twice_double (x : Nat) : Nat :=
  let y := x + x; y * y
...
#check nil#check cons#check map
```
...
#check Lst.cons 0 Lst.nildef as : Lst Nat := Lst.nil
