...
첫 번째 목표는 `exact hp` 명령으로 만족됩니다. `exact` 명령은 단순히 주어진 표현식이 목표를 정확히 채워야 함을 신호하는 `apply`의 변형입니다. 택틱 증명에서 좋은 형태입니다. 왜냐하면 실패는 뭔가 잘못되었음을 나타내기 때문입니다. `apply`보다 더 견고합니다. 왜냐하면 엘레메이터가 적용되는 표현식을 처리할 때 목표의 대상으로 주어진 예상 타입을 고려하기 때문입니다. 하지만 이 경우에는 `apply`도 마찬가지로 작동할 것입니다.
...
`rw` 전술과 `simp` 전술은 [계산적 증명](Quantifiers-and-Equality/#calculational-proofs)에서 간략히 소개되었습니다. 이 섹션과 다음 섹션에서 이들에 대해 자세히 논의합니다.
...
그러나 속성이 적용되면, 이를 영구적으로 제거할 수 있는 방법이 없습니다. 속성이 할당된 파일을 가져오는 모든 파일에 유지됩니다. [속성](Interacting-with-Lean/#attributes)에서 더 논의할 것처럼, `local` 수정자를 사용하여 속성의 범위를 현재 파일이나 섹션으로 제한할 수 있습니다.
...
1. [명제 및 증명](Propositions-and-Proofs/#propositions-and-proofs) 및 [한정자 및 동등성](Quantifiers-and-Equality/#quantifiers-and-equality)의 연습으로 돌아가서, 이제 택틱 증명으로 할 수 있는 많은 것들을 다시 수행하고, 필요에 따라 `rw` 및 `simp`도 사용하십시오.
