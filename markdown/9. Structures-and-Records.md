...
Source: https://lean-lang.org/theorem_proving_in_lean4/Structures-and-Records

# 9. Structures and Records[🔗](find/?domain=Verso.Genre.Manual.section&name=structures-and-records "Permalink")
...
## 9.1. Declaring Structures[🔗](find/?domain=Verso.Genre.Manual.section&name=declaring-structures "Permalink")
...
`open Point``example (a b : α) : x (mk a b) = a :=
rfl``example (a b : α) : y (mk a b) = b :=
rfl`
...
with the same name. Recall from the [Conjunction section](Propositions-and-Proofs/#conjunction) if `p`
...
점 표기법은 레코드의 사영에 접근하기 위해 편리할 뿐만 아니라, 같은 이름의 네임스페이스에 정의된 함수를 적용하기 위해서도 편리합니다. [Conjunction 섹션](Propositions-and-Proofs/#conjunction)을 상기하면, `p`가 `Point` 타입을 가질 때, 표현식 `p.foo`는 `Point.foo p`로 해석됩니다. 단, `foo`의 첫 번째 비암시적 인자가 `Point` 타입을 가져야 합니다. 따라서 표현식 `p.add q`는 아래 예제에서 `Point.add p q`의 약자입니다.

`structure Point (α : Type u) where
x : α
y : α
deriving Repr``def Point.add (p q : Point Nat) :=
mk (p.x + q.x) (p.y + q.y)``def p : Point Nat := Point.mk 1 2``def q : Point Nat := Point.mk 3 4``{ x := 4, y := 6 }#eval p.add q`
...
Point.mk (n * p.x) (n * p.y)``def p : Point Nat := Point.mk 1 2``{ x := 3, y := 6 }#eval p.smul 3`
...
## 9.2. Objects[🔗](find/?domain=Verso.Genre.Manual.section&name=objects "Permalink")
...
`structure Point (α : Type u) where
x : α
y : α``{ x := 10, y := 20 } : Point Nat#check { x := 10, y := 20 : Point Nat }`
...
x : α
y : α
deriving Repr``def p : Point Nat :=
{ x := 1, y := 2 }``{ x := 1, y := 3 }#eval { p with y := 3 }`
...
x : α
y : α
z : α``def q : Point3 Nat :=
{ x := 5, y := 5, z := 5 }``def r : Point3 Nat :=
{ p, q with x := 6 }``example : r.x = 6 := rfl``example : r.y = 2 := rfl``example : r.z = 5 := rfl`

## 9.3. Inheritance[🔗](find/?domain=Verso.Genre.Manual.section&name=inheritance "Permalink")
...
`structure Point (α : Type u) where
x : α
y : α``inductive Color where
| red | green | blue``structure ColorPoint (α : Type u) extends Point α where
c : Color`
...
x : α
y : α
z : α``structure RGBValue where
red : Nat
green : Nat
blue : Nat``structure RedGreenPoint (α : Type u) extends Point α, RGBValue where
no_blue : blue = 0``def p : Point Nat :=
{ x := 10, y := 10, z := 20 }``def rgp : RedGreenPoint Nat :=
{ p with red := 200, green := 40, blue := 0, no_blue := rfl }``example : rgp.x = 10 := rfl``example : rgp.red = 200 := rfl`